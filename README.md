# 🇩🇪🇭🇷 Luca Matteo Špoljarević

*`Privacy‑centric automation lover (Founder @Who2Industries/Developer/FOSS enthusiast)`*

I’m Luca Matteo Špoljarević—a Linux‑savvy system integrator, founder of Who2 Industries, and relentless FOSS advocate—building my slice of the digital world one deliberate step at a time. Every piece of code I write starts from scratch: I map out the problem, sketch the architecture, automate the deployment (Ansible, Docker/Podman, Git), and fine‑tune the final product so it solves real‑life challenges securely and efficiently.

---

### 🧰 Languages and Tools

<div style="display: flex; flex-wrap: wrap; align-items: center; gap: 10px;">
  <img alt="Linux"   width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg">
  <img alt="Bash"    width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bash/bash-original.svg">
  <img alt="ZSH"     width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/zsh/zsh-original.svg">
  <img alt="Podman"  width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/podman/podman-original.svg">
  <img alt="Docker"  width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg">
  <img alt="Git"     width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg">
  <img alt="GitHub"  width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg">
  <img alt="HTML"    width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-plain.svg">
  <img alt="CSS"     width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-plain.svg">
  <img alt="JavaScript" width="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-plain.svg">
</div>

### 📊 Stats

![Spoljarevic's GitHub stats](https://github-readme-stats.vercel.app/api?username=spoljarevic&show_icons=true&theme=gruvbox)